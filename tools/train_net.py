...
from datasets.utils.mixup import Mixup, mixup_contrastive_cross, mixup_contrastive_self, mixup_contrastive_shuffle, mixup_contrastive_cross_2
from datasets.utils.mixup import mixup_contrastive_duplicate, mixup_contrastive_shuffle_duplicate
from datasets.utils.mixup import mixup_contrastive_mixup_shuffle_duplicate, mixup_contrastive_mixup_duplicate, mixup_contrastive_mixup_shuffle
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, indexes, meta) in enumerate(train_loader):
        ...
        if hasattr(cfg.PRETRAIN.CONTRASTIVE, "MIXUP") and cfg.PRETRAIN.CONTRASTIVE.MIXUP:
            if hasattr(cfg.PRETRAIN.CONTRASTIVE, "SHUFFLE") and cfg.PRETRAIN.CONTRASTIVE.SHUFFLE:
                if hasattr(cfg.PRETRAIN.CONTRASTIVE, "DUPLICATE") and cfg.PRETRAIN.CONTRASTIVE.DUPLICATE:
                    inputs = mixup_contrastive_mixup_shuffle_duplicate(cfg, inputs)
                else:
                    inputs = mixup_contrastive_mixup_shuffle(cfg, inputs)
            elif hasattr(cfg.PRETRAIN.CONTRASTIVE, "DUPLICATE") and cfg.PRETRAIN.CONTRASTIVE.DUPLICATE:
                inputs = mixup_contrastive_mixup_duplicate(cfg, inputs)
            else:
                inputs = mixup_contrastive_cross(cfg, inputs)
            # for i in range(inputs['video'].shape[0]):
            #     BaseVideoDataset.visualize_frames(frames=inputs['video'][i][1].squeeze().permute(1,2,3,0), name=str(i))
        elif hasattr(cfg.PRETRAIN.CONTRASTIVE, "SHUFFLE") and cfg.PRETRAIN.CONTRASTIVE.SHUFFLE:
            if hasattr(cfg.PRETRAIN.CONTRASTIVE, "DUPLICATE") and cfg.PRETRAIN.CONTRASTIVE.DUPLICATE:
                inputs = mixup_contrastive_shuffle_duplicate(cfg, inputs)
            else:
                inputs, labels_mix = mixup_contrastive_shuffle(cfg, inputs, labels)
                # for i in range(inputs['video'].shape[0]):
                #     BaseVideoDataset.visualize_frames(frames=inputs['video'][i][1].squeeze().permute(1,2,3,0), name=str(i))
        elif hasattr(cfg.PRETRAIN.CONTRASTIVE, "DUPLICATE") and cfg.PRETRAIN.CONTRASTIVE.DUPLICATE:
            inputs = mixup_contrastive_duplicate(cfg, inputs)
            # for i in range(inputs['video'].shape[0]):
            #     BaseVideoDataset.visualize_frames(frames=inputs['video'][i][1].squeeze().permute(1,2,3,0), name=str(i))
        ...
        optimizer.step()
        if cfg.MODEL.EMA.CHANGE_WITH_LR:
            model_ema.update_ema_decay(
                1 - lr / cfg.OPTIMIZER.BASE_LR * (1-cfg.MODEL.EMA.DECAY) if cur_epoch > cfg.OPTIMIZER.WARMUP_EPOCHS else cfg.MODEL.EMA.DECAY
            )
        if model_ema is not None:
            model_ema.update(model)

        if cfg.PRETRAIN.ENABLE and cfg.MODEL.NAME != 'Contrastive_CE_Model' and cfg.MODEL.NAME != 'PrototypeModel':
            ...
            if isinstance(labels["supervised"], dict):
                ...
                if cfg.MODEL.NAME == 'PrototypeModel':
                    ...
                    if cfg.PRETRAIN.NUM_CLIPS_PER_VIDEO == 2:
                        preds = preds[::2]
                        labels["supervised"] = labels["supervised"][::2]
                elif cfg.VIDEO.HEAD.NAME == 'BaseHeadBNN':
                    preds = preds['pred_mean']
