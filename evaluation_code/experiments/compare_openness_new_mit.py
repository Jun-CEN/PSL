...
def eval_osr(y_true, y_pred):
    ...
    aupr_list = []
    fpr95_list = []
    y_true_ind = y_true[:3783]
    y_pred_ind = y_pred[:3783]
    y_true_ood = y_true[3783:]
    y_pred_ood = y_pred[3783:]
    len_ood = int(y_pred_ood.shape[0] / 10)
    for i in range(10):
        y_true_ood_tmp = y_true_ood[(len_ood*i):(len_ood*(i+1)):1]
        y_pred_ood_tmp = y_pred_ood[(len_ood*i):(len_ood*(i+1)):1]
        y_true = np.concatenate((y_true_ind, y_true_ood_tmp))
        y_pred = np.concatenate((y_pred_ind, y_pred_ood_tmp))
        # open-set auc-roc (binary class)
        auroc = roc_auc_score(y_true, y_pred)
    ...
    aupr = sum(aupr_list) / len(aupr_list)
    fpr95 = sum(fpr95_list) / len(fpr95_list)
...
def eval_uncertainty_methods(result_file, threshold=-1):
    ...
    # open-set evaluation (binary class)
    if threshold > 0:
        if "PSL" in result_file or "openmax" in result_file:
...
def main():
    ...
    # acc, auroc, aupr, fpr95 = eval_uncertainty_methods(ind_openmax, ood_openmax, ind_labels, ood_labels, score='binary')
