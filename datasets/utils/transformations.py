...
class AutoResizedCropVideo(object):
    ...
    def get_crop(self, clip):
        ...
        crop = F.resized_crop(clip.float(), y1, x1, th, tw, self.size, self.interpolation_mode)
        return crop
...
class KineticsResizedCrop(object):
    ...
    def __call__(self, clip):
        if self.idx == -1:
            return self._get_random_crop(clip.float())
        else:
            return self._get_controlled_crop(clip.float())
