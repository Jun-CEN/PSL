...
class Imagenet(BaseVideoDataset):
    ...
    def _get_dataset_list_name(self):
        ...
        if self.cfg.PRETRAIN.ENABLE:
            ...
            elif self.cfg.PRETRAIN.IMAGENET_SS is False:
                if self.cfg.PRETRAIN.IMAGENET_SPLIT is None:
                    name = "imagenet_{}_s{}.txt".format(
                        "train" if self.split == "train" else "val", self.cfg.PRETRAIN.IMAGENET_DATA_SIZE
                    )
                else:
                    name = "imagenet_{}_s{}_{}.txt".format(
                        "train" if self.split == "train" else "val", self.cfg.PRETRAIN.IMAGENET_DATA_SIZE,
                        self.cfg.PRETRAIN.IMAGENET_SPLIT
                    )
            else:
                if self.split == "train":
                    name = "imagenet_train_{}_{}_ss.txt".format(
