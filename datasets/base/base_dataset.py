...
class BaseVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        if self.split in ["train", "val"]:
            ...
            self._num_clips = 1
        elif self.split in ["test", "linear_test_cls", "linear_test_ret", "submission", "test_in", "test_ood", "test_train"]:
    ...
    def _get_video_frames_list(self, vid_length, vid_fps, clip_idx, random_sample=True, idx=0):
        """
        ...
        if self.cfg.AUDIO.ENABLE:
            ...
            if self.cfg.PRETRAIN.ENABLE and hasattr(self.cfg.PRETRAIN, "LONG_SHORT"):
                if idx == 0:
    ...
    def _decode_video(self, sample_info, index, num_clips_per_video=1):
        ...
        if self.split == "train" or self.split == "linear_train_cls":
            ...
            clip_idx = -1
            self.spatial_idx = 0
        elif self.split == "test" or self.split == "test_train" or self.split == "test_in" or self.split == "test_ood" or self.split == "linear_test_cls" or self.split == "linear_test_ret" or self.split == "submission":
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.PRETRAIN.ENABLE:
            ...
            try:
                ...
                # self.visualize_frames(data["video"][0].permute(1, 2, 3, 0), index)
                # self.visualize_frames(data["video"][1].permute(1, 2, 3, 0), index)
            except Exception as e:
                traceback.print_exc()
    ...
    # -------------------------------------- Visualization Utils --------------------------------------
    @staticmethod
    def visualize_frames(frames, name):
        """
        ...
        if not os.path.exists("output/debug/decord_exp"):
            os.mkdir("output/debug/decord_exp")
        print(f"Frames original, max:{frames_vis.max().item()}")
        plt.imsave(f"output/debug/decord_exp/{name}_orig.jpg", frames_vis)
    ...
    def reversed_visualize_frames(self, frames, name):
        ...
        if isinstance(frames, list):
            ...
            if frames.shape[0] == 3:
                ...
                f_vis = frames * torch.tensor(self.cfg.DATA.STD).reshape(-1, 1, 1, 1) + torch.tensor(self.cfg.DATA.MEAN).reshape(-1, 1, 1, 1)
                f_vis = f_vis.clamp(0.,1.).permute(2, 1, 3, 0).reshape(h, t*w, c).detach().cpu().numpy()
                if not os.path.exists("debug/imgn_vit_vis"):
                    os.mkdir("debug/imgn_vit_vis")
                print(f"Frames after, max:{f_vis.max().item()}")
                plt.imsave(f"debug/imgn_vit_vis/{name}.jpg", f_vis)
