...
class Kinetics700flow(BaseVideoDataset):
    def __init__(self, cfg, split):
        super(Kinetics700flow, self).__init__(cfg, split) 
    ...
    def _get_sample_info(self, index):
        ...
        class_ = int(class_)
        video_path = os.path.join(self.data_root_dir, f"{self.cfg.DATA.OPTICAL_FLOW}_of_info", f"{self.split if self.split != 'test' else 'val'}", video_name).split('.')[0] + ".json"
        sample_info = {
            "name": video_name.split('.')[0].replace(' ', '_').replace('(', '0').replace(')', '0'),
            "path": video_path.replace(' ', '_').replace('(', '0').replace(')', '0'),
        ...
        try:
            ...
            for idx in range(len(split_list)-1):
                ...
                if download:
                    ...
                    flow_path = os.path.join(self.data_root_dir, f"{self.cfg.DATA.OPTICAL_FLOW}_of", f"{self.split if self.split != 'test' else 'val'}", video_name, flow_info[idx]["up_name"])
    ...
    def _decode_flow(self, sample_info, index, num_clips_per_video=1):
        ...
        if frames.shape[0] == self.cfg.DATA.NUM_INPUT_FRAMES:
