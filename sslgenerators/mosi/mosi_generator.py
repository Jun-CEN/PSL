...
class MoSIGenerator(object):
    ...
    def __init__(self, cfg, split):
        ...
        self.in_vid_camera_jitter = cfg.PRETRAIN.IN_VID_JITTER_RATIO
        self.fully_random = cfg.PRETRAIN.FULLY_RANDOM
    ...
    def sample_generator(self, data, index):
        ...
        for speed_idx, speed in enumerate(self.speed_set):
            ...
            jitter_x = self.get_in_vid_jitter_matrix(speed_x)
            jitter_y = self.get_in_vid_jitter_matrix(speed_y)
            intermediate_x = (torch.linspace(start_x, end_x, self.num_frames).long() + jitter_x).clamp_(0, w-self.crop_size)
            intermediate_y = (torch.linspace(start_y, end_y, self.num_frames).long() + jitter_y).clamp_(0, h-self.crop_size)
            ...
            # self.visualize_frames(frames_out, index, self.correspondence[speed_idx])
            if self.transform is not None:
                frames_out = self.transform(frames_out)
            if self.static_mask_enable:
                frames_out = self.static_mask(frames_out, self.cfg.PRETRAIN.STATIC_MASK_VERSION)
            # self.visualize_frames(frames_out, index, self.correspondence[speed_idx], idx=2)
    ...
    def get_in_vid_jitter_matrix(self, speed):
        # jitter on each position
        if speed > 0:
            jitter = ((torch.rand(self.num_frames)-0.5) * self.in_vid_camera_jitter * self.crop_size * abs(speed)).long()
        else:
            jitter = ((torch.rand(self.num_frames)-0.5) * self.in_vid_camera_jitter * self.crop_size).long()
        return jitter
    ...
    def static_mask(self, frames, version):
        """
        Applys static mask to the generated pseudo motion sequence. 
        Args: 
            frames (Tensor): shape of (C,T,H,W)
            version (int): the version of the static mask
        """
        if version == 0:
            mask_id = random.randint(0, 4) # 0: left, 1: right, 2: top, 3: bottom. 4: no cover
            c, t, h, w = frames.shape
            rand_t = random.randint(0, t-1)
            if mask_id == 0:
                frames[:, :, :, :w//2] = frames[:, rand_t, :, :w//2].unsqueeze(1)
            elif mask_id == 1:
                frames[:, :, :, w//2:w] = frames[:, rand_t, :, w//2:w].unsqueeze(1)
            elif mask_id == 2:
                frames[:, :, :h//2, :] = frames[:, rand_t, :h//2, :].unsqueeze(1)
            elif mask_id == 3:
                frames[:, :, h//2:h, :] = frames[:, rand_t, h//2:h, :].unsqueeze(1)
            return frames
        elif version == 1:
            """
            Image segment id
            ...
            ---------
            """
            mask_id = random.randint(0, 5) # 0: no mask, 1: id1 move, 2: id2 move, 3: id3 move, 4: id4 move, 5: center move
            c, t, h, w = frames.shape
            rand_t = random.randint(0, t-1)
            if mask_id == 0:
                frames_out = frames
                ...
                frames_out[:, :, :h//2, :w//2] = frames[:, :, :h//2, :w//2]
            elif mask_id == 2:
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, :h//2, w//2:w] = frames[:, :, :h//2, w//2:w]
            elif mask_id == 3:
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, h//2:h, :w//2] = frames[:, :, h//2:h, :w//2]
            elif mask_id == 4:
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, h//2:h, w//2:w] = frames[:, :, h//2:h, w//2:w]
            elif mask_id == 5:
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, h//4:3*h//4, w//4:3*w//4] = frames[:, :, h//4:3*h//4, w//4:3*w//4]
            return frames_out
        elif version == 2:
            """
            Combination of version 0 and 1.
            Image segment id:
            ---------
            | 1 | 2 |
            ---------
            ...
            ---------
            """
            mask_id = random.randint(0, 9)  # 0: global movement, 1: ID[1,2] move, 2: ID[2,4] move, 3: ID[3,4] move, 4: ID[1, 3] move
                                            # 5: id[1] move, 6: id[2] move, 7: id[3] move, 8: id[4] move, 9: center move
            c, t, h, w = frames.shape
            rand_t = random.randint(0, t-1)
            if mask_id == 0:
                frames_out = frames
                ...
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, :h//2, :] = frames[:, :, :h//2, :]
            elif mask_id == 2:
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, :, w//2:w] = frames[:, :, :, w//2:w]
            elif mask_id == 3:
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, h//2:h, :] = frames[:, :, h//2:h, :]
            elif mask_id == 4:
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, :, :w//2] = frames[:, :, :, :w//2]
            elif mask_id == 5:
                frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
                frames_out[:, :, :h//2, :w//2] = frames[:, :, :h//2, :w//2]
            elif mask_id == 6:
            ...
            """
            Biased version 1 towards center moves.
            Image segment id
            ---------
            | 1 | 2 |
            ---------
            ...
            ---------
            """
            mask_id = random.randint(0, 7) # 0: no mask, 1: id1 move, 2: id2 move, 3: id3 move, 4: id4 move, 5: center move, 6: center move, 7: center move
            c, t, h, w = frames.shape
            rand_t = random.randint(0, t-1)
            if mask_id == 0:
                frames_out = frames
            ...
            """
            Random mask with fixed size.
            """
            c, t, h, w = frames.shape
            rand_t = random.randint(0, t-1)
            mask_size_x, mask_size_y = [w//2, h//2]
            ...
            """
            Fixed mask position with random size.
            """
            c, t, h, w = frames.shape
            rand_t = random.randint(0, t-1)
            mask_size_ratio = random.uniform(self.mask_size_ratio[0], self.mask_size_ratio[1])
            mask_size_x, mask_size_y = [int(w*mask_size_ratio), int(h*mask_size_ratio)]
            start_x = (w-mask_size_x)//2
            start_y = (h-mask_size_y)//2
            frames_out = frames[:, rand_t].unsqueeze(1).expand(-1, t, -1, -1).clone()
            frames_out[:, :, start_y:start_y+mask_size_y, start_x:start_x+mask_size_x] = frames[
                :, :, start_y:start_y+mask_size_y, start_x:start_x+mask_size_x
            ]
    ...
    def visualize_frames(self, frames, index, move_dir, idx=1):
        if not os.path.exists("output/visualization/pseudo_motion"):
            os.makedirs("output/visualization/pseudo_motion")
        if len(frames.shape) == 4:
            t,h,w,c = frames.shape
            # for frame_idx in range(t):
            if not os.path.exists("output/visualization/pseudo_motion/{}/".format(index)):
                os.makedirs("output/visualization/pseudo_motion/{}/".format(index))
            ...
            if idx == 1:
                frames_vis = frames.permute(1,0,2,3).reshape(frames.shape[1], -1, frames.shape[3]).detach().cpu().numpy()
            elif idx == 2:
                frames_vis = frames.permute(2,1,3,0).reshape(frames.shape[2], -1, frames.shape[0]).detach().cpu().numpy()
            plt.imsave("output/visualization/pseudo_motion/{}/move_{}_{}_{}.jpg".format(index, move_dir[0], move_dir[1], idx), frames_vis)
...
@SSL_GENERATOR_REGISTRY.register()
class MoSIGeneratorWithRandom_EXP(object):
    """
    Generator for pseudo camera motions with static masks in MoSI.
    ...
    def initialize_speed_set(self):
        ...
        Args:
            random_mode: 
                X denotes randomize scenarios where x is not zero
                Y denotes randomize scenarios where y is not zero
                x denotes x without randomization
                y denotes y without randomization
        """
        # Generate all possible speed combinations
        self.speed_all = []
        _zero_included = False
            ...
            for y in self.speed_range:
                ...
                if x==0 and y==0:
                    if _zero_included:
                        ...
                        _zero_included = True
                if self.cfg.PRETRAIN.DECOUPLE and x*y != 0: # if decouple, then one of x,y must be 0
                    continue
                self.speed_all.append(
                    [x, y]
                )
        ...
        if self.cfg.PRETRAIN.DECOUPLE:
            ...
            if "x" in self.cfg.PRETRAIN.DATA_MODE and "y" in self.cfg.PRETRAIN.DATA_MODE:
                self.speed_set = self.speed_all
                self.fixed_flags = [False for i in range(len(self.speed_set))]
            else:
                raise NotImplementedError("Not supported for data mode {} when DECOUPLE is set to true.".format(self.cfg.PRETRAIN.DATA_MODE))
        if self.cfg.PRETRAIN.DECOUPLE and not self.cfg.PRETRAIN.ZERO_OUT:
            # add speed=0
    ...
    def sample_generator(self, data, index):
        ...
        for speed_idx, speed in enumerate(self.speed_set):
            ...
            h, w, c = selected_frame.shape
            speed_x, speed_y = speed
            if self.fixed_flags[speed_idx] and self.fully_random:
                # if fully random, samples can be selected from the whole image
                # as the selected video will not move in spatial dimensions
                start_x = random.randint(0, w-self.crop_size)
                start_y = random.randint(0, h-self.crop_size)
            else:
                start_x, end_x = self.get_crop_params(speed_x/(self.num_speeds//2), w)
                start_y, end_y = self.get_crop_params(speed_y/(self.num_speeds//2), h)
            ...
            if self.fixed_flags[speed_idx]:
                # frames_out = torch.repeat_interleave(
                #     selected_frame[:, start_y:start_y+self.crop_size, start_x:start_x+self.crop_size].unsqueeze(1), 
                #     self.num_frames, 1
                # )
