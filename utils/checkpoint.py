...
def save_checkpoint(path_to_job, model, model_ema, optimizer, epoch, cfg, model_bucket=None):
    ...
    prototypes = model.module.prototypes if hasattr(model.module, "prototypes") else None
...
def load_checkpoint(
    ...
    if 'prototypes' in checkpoint.keys():
        model.module.prototypes = checkpoint['prototypes'].cuda()
        logger.info("Load prototypes: {}".format(checkpoint['prototypes']))
    
    if "model_state" not in checkpoint.keys():
        checkpoint_dict = checkpoint["state_dict"]
        for k, v in list(checkpoint_dict.items()):
            if "base_model" in k.split("."):
                new_k = k.replace("module", "backbone")
            elif k == "module.new_fc.weight":
                new_k = "head.out.weight"
            elif k == "module.new_fc.bias":
                new_k = "head.out.bias"
            
            # print(new_k, k)
            
            checkpoint_dict[new_k] = checkpoint_dict.pop(k)
    ...
    if inflation:
        ...
        for k, v in ms.state_dict().items():
            if k in checkpoint["model_state"].keys():
                if checkpoint["model_state"][k].size() != v.size():
